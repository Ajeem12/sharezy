          ...
          <ul className="hidden md:flex items-center space-x-8">
            {navItems.map(({ id, label, icon, to }) => (
              <motion.li key={id} variants={itemVariants}>
                <Link to={to} className="flex items-center gap-2 px-3 py-2 text-gray-700 hover:text-blue-600">
      ...
      {isDropdownOpen && (
        ...
        >
          {isLoggedIn ? accountItems.map((item) => {
            ...
            return (
              <motion.div key={item.id} variants={dropdownVariants}>
                <Link to={item.to} className="flex items-center rounded-lg gap-3 px-4 py-3 text-sm hover:bg-blue-50">
                  <span className="text-blue-500">{item.icon}</span>{item.label}
                </Link>
              </motion.div>
            );
            ...
            <motion.div key={item.id} variants={dropdownVariants}>
              <Link to={item.to} className="flex items-center gap-3 px-4 py-3 text-sm hover:bg-blue-50">
                <span className="text-blue-500">{item.icon}</span>{item.label}
              </Link>
            </motion.div>
          ))}
        ...
        >
          {navItems.map(({ id, label, icon, to }) => (
            <motion.li key={id} variants={mobileItemVariants}>
              <Link to={to} className="flex items-center gap-3 py-3 text-gray-700 hover:text-blue-600">
                <span className="text-blue-500 text-lg">{icon}</span>{label}
              </Link>
            </motion.li>
          ))}
          {(isLoggedIn ? accountItems : authItems).map(({ id, label, icon, to }) => {
            if (label === "Logout") {
              return (
                <motion.li key={id} variants={mobileItemVariants}>
                  <button
